...
function formatMetrics(metrics) {
  return (
    <div>
      ...
      <div>
        duration: {helpers.formatNumber(metrics.requests.duration)} s (p99)
      </div>
    </div>
  );
}
...
export default ({ name }) => {
  ...
  const warning = consumerGroupInfo.filter(
    (cg) => cg.performance === "WARNING"
  );
  const style =
    critical.length > 0
      ? styles.critical
  ...
  return (
    <div
      key={name}
      className={`${styles.service} ${style} ${styles.infoDriver}`}
    >
      <h1>{name}</h1>
      <span>
